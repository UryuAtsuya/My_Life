...
- サービス名: `XGuard`
- 想定価格: `Free診断 / 1,980円 Personal / 2,980円 Pro / 4,980円 Concierge検証枠`
- 実装方針: Next.js 14 + Node.js Express + Supabase + Stripe + X API
- リポジトリ案: `xguard/` モノレポ
- GitHub実装repo: `UryuAtsuya/Xguard`
- デプロイ想定: フロントVercel、バックエンドRailway
- 現段階: backend-first prototype実装中。production release gate検証中。

## GitHub同期ルール

- XGuardの実装コードは `/Users/uryuatsuya/XGuard/xguard` を作業場所にする。
- `develop` をstaging、`main` をproductionとして扱う。
- 通常の実装は `develop`、または `develop` から作成した短命な `feature/*` で行う。
- meaningfulなコーディング作業が完了し、検証が通ったら、原則として `UryuAtsuya/Xguard` の `feature/*` または `origin/develop` へcommit/pushまで行う。
- staging検証済みの同一commitだけを `develop` から `main` へpull requestで昇格し、実装変更を `main` へ直接pushしない。
- 現在のprivate repositoryプランではGitHubのbranch protectionを利用できないため、`main` 直push禁止はautomation、CI、運用規約で担保する。remote側での強制ではないことを前提に扱う。
...
1. `/Users/uryuatsuya/XGuard/xguard` の local `7e33e9f Guard production OAuth callback boundary` と remote の `c4403d8` / `9ac4f2f` 関係を確認し、force pushなしで整合する。
2. OAuth configured modeで実X token exchangeを検証し、取得tokenのsubject/account検証後だけsessionを発行する。live credentials不足時はproduction mock callback/session発行禁止gateを証跡化する。
3. 実Supabase/Postgres integration testを走らせ、usage ledger境界の証跡を残す。
4. `docs/API_COST_MODEL.md` と `docs/COMPLIANCE.md` のrelease gateをCI/runtime停止条件へ落とす。
...
## 2026-06-04 evening code review update

- 指定パス `/Users/uryuatsuya/XGuard/xguard` は `HEAD=origin/main=394a3c3 Add OAuth diagnostic HTTP boundary tests`、working tree clean。
- 診断endpointの無認証公開は解消済み。HTTP境界テストとAPI仕様の認証列を追加し、`UryuAtsuya/Xguard` `origin/main` へpush済み。
- Review P1はOAuth CSRF/replay、backup/proof認証・所有権、実Supabase/Postgres、商用compliance gate。
- 検証は `git diff --check`, `git diff --cached --check`, `tsc --noEmit`, `npm run test` pass。全testは `50 passed / 4 skipped`。build/checkは `dist/` write `EPERM`。
- production releaseはNo-Goを維持し、次はOAuth安全化、API所有権境界、実DB/cost/complianceを閉じる。
...
## 2026-05-30 morning research update

- 調査メモ: `requirements/2026-05-30-x-ban-research.md`
- 朝会引き継ぎ: `notes/2026-05-30-morning-planning.md`
- 今日の優先は、実Supabase/Postgres migration test、real OAuth configured mode、Developer Console原価確認の順にする。
- v0初期scopeは `tweet.read`, `users.read`, `offline.access` のみに維持する。
