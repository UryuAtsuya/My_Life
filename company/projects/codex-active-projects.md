...
|---|---|---|---|
| 1 | X BAN Recovery Storage | `feature/supabase-proof-page-transaction-store` / XGuard commit `31482ed` / PR #27 / proof page transaction store実装 / production No-Go | PR #27をreviewし、実 Supabase/Postgresで `update_proof_page_visibility_and_record_content_compliance_event` のservice-role実行、rollback、owner/account整合性を検証する |
...
| `x-ban-recovery-storage` | `feature/supabase-proof-page-transaction-store` / XGuard commit `31482ed` / PR #27 / proof page transaction store実装 / production No-Go | XアカウントBAN後に新アカウントで再起動できるよう、平常時からXデータをDB保管し、証明ページと復元導線を作るサービス。 | PR #27をreviewし、実 Supabase/Postgresで `update_proof_page_visibility_and_record_content_compliance_event` のservice-role実行、rollback、owner/account整合性を検証する。 |
