...
- XGuardの通常作業branchを `develop`、stagingを `develop`、productionを `main` に固定した。
- 朝・昼・夜のactive Codex automationを `develop` 基準へ更新した。
- 昼automationはsubagent toolを先に確認し、Implementation、Review、Verification、Syncを重複しない範囲で割り当てる。
- subagent spawnが引数制約などで失敗した場合は、原因を修正して1回だけ再試行する。利用不可、usage limit、再試行失敗時だけ順次role passへ切り替える。
- agentの最終判断、差分統合、commit、pushはmain coordinatorが保持する。
- 実疎通では最初の `fork_context` と `agent_type` の併用が拒否されたが、文脈をpromptへ明示して再実行すると複数agentを起動できた。
- 各agentはagent id、担当file、base SHAを記録し、write agentは分離されたworktree/branchを使う。MyLife共有文書はmain coordinatorだけが編集する。
- 各roleには必須成果物を定義し、Syncは書き込み役ではなく `Sync planner` としてMyLife更新案を返す。
- commit前に全agentを完了、失敗、timeoutへ確定し、稼働中agentを残さない。timeoutしたroleだけ順次fallbackする。
- agent結果は `role | mode | agent_id | base_sha | owned_paths | status | artifact | fallback_reason` で記録する。
- 順次role passは同一coordinatorによるfallbackであり、独立reviewとして数えない。
- private repositoryの現在のGitHubプランではbranch protectionを利用できないため、`main` 直push禁止はremoteで強制されていない。automation、CI、運用規約によるguardとして扱う。
