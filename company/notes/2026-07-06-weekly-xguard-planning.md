...
- 既存差分: `backend/src/config/runtimeConfig.ts`, `backend/src/repositories/oauthStateRepository.ts`, `backend/src/repositories/supabaseOAuthStateHttpStore.ts`, `backend/src/__tests__/oauthStateRepository.test.ts`, `backend/src/__tests__/supabaseOAuthStateHttpStore.test.ts`, `.playwright-cli/`, `output/playwright/` は今回対象外として触らない。
